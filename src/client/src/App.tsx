...
import { Elements, addEdge, removeElements, Edge, Node, Connection, isNode, isEdge, FlowElement, ArrowHeadType } from 'react-flow-renderer';
...
const App: React.FC = () => {
	...
	const onElementsRemove = async (elementsToRemove: Elements) => {
	    ...
	    for (const e of sortedElementsToRemove) {
	        if (isNode(e)) {
	            await nodeService.deleteNode(e).catch((e: Error) => console.log('Error deleting Node', e))
	        }
	        else if (isEdge(e)) {
